...
function registerNewUser(admin,client){
    let caUrl = ORGS['org1'].ca;
    let caClient = new copService(caUrl, null /*defautl TLS opts*/, '' /* default CA */, client.getCryptoSuite());
    
   return  caClient.register({
						enrollmentID: 'sankalp',
						affiliation: 'org1' + '.department1'
					}, admin).then((secret) => {
					enrollmentSecret = secret;
					logger.debug('sankalp' + ' registered successfully');
					return caClient.enroll({
						enrollmentID: 'sankalp',
						enrollmentSecret: secret
					});
				}, (err) => {
					logger.debug('sankalp' + ' failed to register');
					console.log( err);
					//return 'Failed to register '+username+'. Error: ' + err.stack ? err.stack : err;
				})
}

var enrollUser=function(username,org,secret,req,res){
	let caUrl = ORGS['org1'].ca;
